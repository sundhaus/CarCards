...
final class CardRenderer {
    ...
    func portraitCard(for card: AnyCard, width: CGFloat = 300) -> UIImage? {
        ...
        // Render landscape first, then rotate
        let landscapeHeight = width * 16.0 / 9.0 // this becomes portrait height after rotation
        guard let landscape = renderLandscape(card: card, sourceImage: card.thumbnail ?? card.image ?? UIImage(), height: width) else { return nil }
        
        // Rotate 90° clockwise to get portrait
        let rendered = rotateImage(landscape, degrees: 90)
        if let rendered {
